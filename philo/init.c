...
/*   Updated: 2023/09/18 02:13:31 by cahn             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	allocate_datas(t_base *base)
{
	...
	memset(base->philo, 0, sizeof(t_philo) * base->num_of_philo);
	return (1);
}

int	init_value(t_base *base)
{
	int	i;

	i = 0;
	base->complete_eat_cnt = 0;
	base->finish = 0;
	...
	while (i < base->num_of_philo)
	{
		...
		pthread_mutex_init(&base->philo[i].last_eat_flag, NULL);
