...
/*   Updated: 2023/09/24 00:56:29 by cahn             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	take_rfork(t_philo *philo, t_base *base)
{
	...
	if (!rf && !print_state_with_finish(get_millisec(base->start), \
			philo->id + 1, "has taken a fork", base))
	{
...
int	take_lfork(t_philo *philo, t_base *base)
{
	...
	if (!lf && !print_state_with_finish(get_millisec(base->start), \
		philo->id + 1, "has taken a fork", base))
	{
